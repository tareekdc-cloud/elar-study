...
const books = [
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        modelAnswer: "In The Lightning Thief, Rick Riordan reframes Percy's ADHD and dyslexia as divine gifts rather than disabilities, developing the theme that qualities which make someone feel broken in one context can be exactly what make them powerful in another. Percy's dyslexia — which has gotten him expelled from school after school — is revealed to exist because his brain is wired for ancient Greek, the language of his divine heritage. His ADHD, dismissed by teachers as an inability to focus, is reframed as battlefield instincts built for survival. During the encounter with the Minotaur, Percy's impulsive, hyperactive energy is precisely what allows him to act when a more cautious fighter might hesitate and be killed. Riordan's argument is not simply that being different is fine — it is more specific than that: the standards by which Percy was judged his whole life were never designed for who he actually is. A world built for demigods sees his 'flaws' as strengths, which is exactly the theme Riordan wants readers with similar struggles to take away.",
      },
      {
        type: "Multiple Choice",
  ...
  },
  {
    ...
    vocab: [
      { word: "Lethargy", syllables: "leth·ar·gy", definition: "A state of sluggish disinterest, lack of energy, and absence of enthusiasm for anything.", context: "Milo approached everything with lethargy — not because he disliked anything in particular, but because he couldn't find the energy to care about anything at all.", staarNote: "STAAR tests 'lethargy' by surrounding it with descriptions of inaction, disinterest, and passive behavior. It is different from laziness — lethargy implies a deeper, more pervasive lack of energy." },
      { word: "Superfluous", syllables: "su·per·flu·ous", definition: "More than what is needed or wanted; unnecessary and excessive.", context: "The Whether Man spoke in superfluous circles — every sentence contained more words than necessary and arrived at no useful conclusion.", staarNote: "Context clues for 'superfluous' include excessive amounts of something, repetition, and the sense that things could be removed without any loss." },
      { word: "Pedantic", syllables: "pe·dan·tic", definition: "Overly concerned with minor details or rules, especially in teaching; excessively focused on formal correctness.", context: "The inhabitants of Digitopolis could be pedantic about numbers — insisting on precise calculations even when a rough estimate would have worked perfectly well.", staarNote: "STAAR tests 'pedantic' by showing a character who corrects others unnecessarily, focuses on trivial details, or prioritizes rules over understanding." },
      { word: "Absurd", syllables: "ab·surd", definition: "Wildly unreasonable, illogical, or inappropriate in a way that seems laughable.", context: "The laws of the Lands Beyond were delightfully absurd — jumping to conclusions was an actual island, and half-baked ideas were served as literal food.", staarNote: "Context clues for 'absurd' include situations that defy logic, things that shouldn't be possible treated as normal, and a mix of humor and impossibility." },
      { word: "Trivial", syllables: "triv·i·al", definition: "Of little value or importance; not worth serious attention.", context: "The Terrible Trivium specialized in trivial tasks — jobs that felt purposeful but produced nothing and consumed enormous amounts of time.", staarNote: "STAAR tests 'trivial' by contrasting it with things that are meaningful or significant. Context clues include wasted time, pointless activity, and things that don't lead anywhere." },
      { word: "Daunting", syllables: "daunt·ing", definition: "Seeming difficult to deal with in a way that causes hesitation or discouragement.", context: "The quest to rescue Rhyme and Reason seemed daunting at first — the obstacles were enormous and the path was completely unclear.", staarNote: "Context clues for 'daunting' include hesitation, descriptions of difficulty, and a character feeling uncertain about whether they can succeed." },
      { word: "Illuminate", syllables: "il·lu·mi·nate", definition: "To light up; also, to help clarify or explain something that was previously unclear.", context: "Every encounter on Milo's journey seemed to illuminate something — not just facts, but a new way of looking at the world around him.", staarNote: "STAAR uses 'illuminate' in both its literal sense (light) and its figurative sense (making something clear). Context determines which meaning applies." },,,
      { word: "Sanctuary", syllables: "sanc·tu·ar·y", definition: "A place of refuge or safety; a place where one is protected from pursuit or danger.", context: "For Milo, the Lands Beyond became a kind of sanctuary — a place where curiosity was rewarded rather than dismissed as impractical or strange.", staarNote: "Context clues for 'sanctuary' include safety, refuge, a place distinct from the troubles of ordinary life, and a character finding peace or protection they couldn't find elsewhere." },
      { word: "Ephemeral", syllables: "e·phem·er·al", definition: "Lasting for a very short time; passing quickly and not permanent.", context: "The Soundkeeper warned Milo that sound, like all beautiful things, was ephemeral — it had to be experienced in the moment or it would be lost forever.", staarNote: "Context clues for 'ephemeral' include descriptions of things fading, not lasting, needing to be experienced now, and the sadness of beauty that passes quickly." },
      { word: "Revelation", syllables: "rev·e·la·tion", definition: "A surprising and previously unknown fact that is made known; the act of making something hidden visible.", context: "Each stop on Milo's journey brought a new revelation — not just about the Lands Beyond, but about the ordinary world he had left behind without truly seeing it.", staarNote: "STAAR uses 'revelation' both as a plot event (a character learns something new) and as a theme (a journey that changes how someone sees the world). Look for before-and-after contrasts in understanding." },
      { word: "Whether", syllables: "wheth·er", definition: "A word introducing a choice or doubt between two possibilities, used to ask if something is the case.", context: "The Whether Man insists it is far more important to know whether there will be weather than what the weather will be — confidently discussing something that, examined closely, says nothing at all.", staarNote: "STAAR uses this word to test whether students can recognize wordplay: 'whether' (a word of doubt or choice) sounds identical to 'weather,' and the joke depends on noticing that distinction." },
    ],
    questions: [
      {
        type: "Multiple Choice",
  ...
  },
  {
    ...
    questions: [
      ...
      {
        type: "Multiple Choice",
        skill: "Character Development — Gretel",
        staar: true,
        map: true,
        question:
      ...
      <div onClick={() => setFlipped(f => !f)} style={{
        ...
        {!flipped ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 180, textAlign: "center", paddingTop: 24 }}>
            {mode === "word" ? (
              <>
                <div style={{ fontSize: 42, fontFamily: "'Georgia', serif", fontWeight: 900, color: book.color, marginBottom: 6, letterSpacing: -1 }}>{card.word}</div>
                {card.syllables && (
                  <div style={{ fontSize: 14, color: "#95A5A6", fontFamily: "monospace", letterSpacing: 2, marginBottom: 10 }}>
                    {card.syllables}
                  </div>
                )}
...
// Find which of this book's vocab words (if any) is being tested in a Vocabulary in Context
// question. Most questions wrap the target word in single quotes (e.g. "...as 'tenacious'
// in the way..."), but some quote a longer phrase instead (e.g. "...is 'far more important to
// know whether...'"). Matching directly against the book's known vocab list handles both cases
// reliably, since we don't need to guess where the word is — we just check which known word
// actually appears in the question text.
function extractVocabWord(questionText, vocabList) {
  const lowerQ = questionText.toLowerCase();
  if (vocabList && vocabList.length) {
    const found = vocabList.find(v => lowerQ.includes(v.word.toLowerCase()));
    if (found) return found.word.toLowerCase();
  }
  // Fallback: simple quoted-single-word extraction, for cases with no vocabList available
